...
import com.bubi.creator.entity.PdmUser;
import com.bubi.creator.entity.PhysicalDiagram;
...
public class ParserPdmService{
	...
	public Pdm pdmParser(String projectType,String rootPath,String fileName) throws Exception{
		String pdmFile = null;
		//生成类，资源文件，mybatis 配置文件
		if (projectType.equals("maven")){
			pdmFile = rootPath + File.separator 
					+ "src" + File.separator 
					+ "main" + File.separator 
		...
		logger.debug("开始解析数据库设计文件:" + pdmFile);
	 ...
	 * @throws Exception
	 */
	public ArrayList<PdmTable> tableParser(Node node) throws Exception{
		...
		for(Object tableNodeObj:node.selectNodes("c:Tables/o:Table")){
			...
			String code = parseCode(1,originalTableCode);
